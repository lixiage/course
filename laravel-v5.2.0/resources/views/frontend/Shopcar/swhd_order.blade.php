...
<div class='body-main'>
	...
	<textarea class="storageCart" style="display:none">
        ...
        <form action="{{ url('swhd_payment') }}" method="post">
        @foreach($arr as $key => $val)
        <div class="detail-box">
            <ul>
                <li class="clearfix js-item-cart js-each-136" data-type="1" data-typeid="136" data-goodsid="362">
                    <a href="" target="_blank">
                        <input type="hidden" value="{{ asset($val['cour_image']) }}" name="cour_image[]"/>
                        <img src="{{ asset($val['cour_image']) }}" alt="" class="l">
                    </a>
                    <div class="text-info-box l">
                        <!-- type为类型 1实战购买 2实战续费 4职业路径购买 5职业路径续费-->
                        <a href="" target="_blank"><p class="info-title">{{ $val['cour_name'] }}</p></a>
                    ...
                    <input type="hidden" value="{{ $val['goods_price'] }}" name="goods_price[]"/>
                    <p class="info-price l"><em>￥</em><span>{{ $val['goods_price'] }}</span></p>
                </li>
            </ul>
        </div>
        @endforeach
        <div class="pay-box clearfix">
            <div class="goods-total-price-box clearfix">
                <p class="r rw price-num">
                    <em>￥</em><span id="js-actual-price">{{ $sum_price }}</span>
                </p>
                <p class="r price-text">共<span>{{ $num }}</span>件商品，商品总金额：</p>
            </div>
            <div class="pay-price-box clearfix">
                <p class="r rw price">
                    <em>￥</em><span id="js-pay-price">{{ $sum_price }}.00</span>
                </p>
                <p class="r price-text">应付：</p>
            </div>
            <div class="pay-account-box clearfix">
                <p class="pay-account r">购买账号：<span>{{ $user[0]->username }}</span></p>
            </div>

            <a href="{{ url('swhd_payment',['order_id'=>$order_id]) }}" class="r moco-btn moco-btn-red submit-btn js-pay-submit" data-typecate="normal" >提交订单</a>
        </div>
        </form>
    </div>
